// theme-switcher.js - Переключение между светлой и темной темой
document.addEventListener('DOMContentLoaded', function() {
    const themeToggleBtn = document.querySelector('.theme-toggle-btn');
    
    // Проверка предпочтений пользователя в localStorage, если не установлена тема в сессии
    if (!document.documentElement.getAttribute('data-theme')) {
        ...
        if (savedTheme) {
            ...
            // Если пользователь предпочитает темную тему на уровне ОС
            document.documentElement.setAttribute('data-theme', 'dark');
            updateThemeIcon('dark');
        }
    }
    
    // Обработчик изменения медиа-запроса системных предпочтений
    ...
    // Обновление иконки в соответствии с текущей темой
